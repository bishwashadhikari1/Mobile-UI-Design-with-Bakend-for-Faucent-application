...
contractrouter.post("/filter", authenticate, async (req, res) => {
...
contractrouter.put("/:id", authenticate, async (req, res) => {
  ...
  if (req.user.user.role !== "employer") {
    return res.status(403).json({ error: "You are not authorized" });
  }
  ...
  try {
    ...
    if (
      ...
      contract.transaction_amount_per_transfer =
        (amountToDivide * 60) / contract.duration;
    }
